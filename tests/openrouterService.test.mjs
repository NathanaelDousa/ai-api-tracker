...
test("openrouter: uses api-reported limit as budget ceiling", async () => {
  mockFetch(200, { data: { usage: 5.00, limit: 20.00, is_free_tier: false } });
  const d = await fetchOpenRouterUsage(BASE_SETTINGS);

  assert.equal(d.monthlyCost,     5.00);
  assert.equal(d.budgetTotal,     20.00);
  assert.equal(d.budgetRemaining, 15.00);
});

test("openrouter: falls back to configured budget when no hard limit", async () => {
  mockFetch(200, { data: { usage: 3.50, limit: null, is_free_tier: false } });
...
test("openrouter: remaining never goes below zero", async () => {
  mockFetch(200, { data: { usage: 25.00, limit: 20.00, is_free_tier: false } });
  const d = await fetchOpenRouterUsage(BASE_SETTINGS);
...
test("openrouter: throws bad-api-key on 401", async () => {
  mockFetch(401, { error: { message: "Unauthorized", code: 401 } });
  await assert.rejects(
